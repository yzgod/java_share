...
docker exec  -it【交互式方式执行命令】容器id  命令
...
CMD不一定会被执行，docker run yz.com/test_cmd:0.1 ls 后面跟一个命令之后，会替换cmd命令。
...
# 绑定网络
docker network connect my-bridge redis6379
docker network connect my-bridge redis6380

# 新开个centos容器
docker run -it --name xx1 xx bash

# 绑定到my-bridge
docker network connect my-bridge xx1

# 进入xx1
docker exec -it containerId bash
ping redis6380
PING redis6380 (172.18.0.3) 56(84) bytes of data.
64 bytes from redis6380.my-bridge (172.18.0.3): icmp_seq=1 ttl=64 time=0.067 ms
```
